...
FBFriends::FBFriends (const FBFriends& other) { //Copy function
    ...
    for (start(); is_item(); advance()) {
        data[counter] = other.data[counter];
        counter++;
    }
...
void FBFriends::insert(const MyFriend& f) { //Adjusts and inserts an object before current index 
    ...
    if (is_item() == true) {
    for (int i = used; i > current_index; i--) { //Reverse shuffle function (starts at end of list), not compatible with given interator functions
        data[i] = data[i - 1];
    }
    data[current_index] = f;
    used++;
    } else {
    for (int i = used; i > 0; i--) { //Reverse shuffle function (starts at end of list), not compatible with given interator functions
       data[i] = data[i - 1];
    }
    data[0] = f;
    used++;
...
void FBFriends::show_all(std::ostream& outs) const { //Runs through the entire friends' list and displays all objects
    ...
    for (int i = 0; i < used; i++) { //Iterator functions not useable in const functions
        data[i].output(outs);
    }
}
...
while (!complete) {
    complete = true;
    for (int i = used - 1; i > 0; i--) { //Reverse bubble sort function, not compatible with given interator functions
