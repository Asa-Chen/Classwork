...
functions that you will be creating in your Checkbook class.
    You are NOT to alter this file
...
int main(){
    ...
    cin>>user; //username is the file that the program opens
    ...
    if(!ifs.fail()){  //checks to see if the file opens
		mychecks.load_from_file(ifs);
		ifs.close(); //need to write a function called load_from_file and takes an input stream as argument
    }
    ...
    do{
		...
		switch(choice){
			case 1:
				...
				cin>>depamount; //each one of these fucntions need to be written inside checkbook class. JUST UNCOMMENT, nothing else!
	...
	ofs.open(userfile.c_str()); //opens same file used in the beginning, overwrites it, the closes it
	if(!ofs.fail()){
		mychecks.save(ofs);
	    ofs.close();
	}
